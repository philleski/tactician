...
/**
 * <p>This class implements a Zobrist hash that is used to map a chess position to a 64-bit long.
 * Zobrist hashing works by assigning a 64-bit long to each piece type and square. This hash is
 * typically used by first assigning 0L to a positionHash variable. When a piece moves onto or
 * off of a given square, positionHash is xor'd with the specific mask corresponding to that
 * piece and square. That way if the same position comes about through a different series of moves
 * it will generate the same position hash. This is very helpful for memoization: storing what was
 * learned about a position and retrieving it when the position is encountered again.
 * 
...
public class PositionHasher {
	 ...
	 * for the white bishop on e3. The idea is every time a bishop either gets onto or leaves e3,
	 * we xor the running position hash with this value.
	...
	/**
	 * Returns the mask for a given color, a piece, and two different squares. This is a
	 * convenience method that simply xors the two, and it is typically used by a piece moving from
	...
	/**
	 * Returns the mask for an en passant target square. Note that we only need to take into
	 * account the file of the en passant square since for example e3 and e6 can be disambiguated
	 * by tracking the turn mask through {@link #getMaskTurn()} in the position hash. We use the
	 * white pawn mask's first rank for storage since pawns cannot be on either the first nor
	 * eighth ranks.
	 * @param enPassantTarget the value of the destination square for en passant capture on the
	 *        next turn
	 ...
	 * we use the eighth rank, otherwise we use the first rank. If white queenside is set we use
	 * the first four files (queenside), otherwise we use the kingside. If black kingside is set we
	 * use the abef files, otherwise we use cdgh. If black queenside is set we use the aceg files,
	 ...
	 * elements and corresponds to the square for the color and piece. The en passant target,
	 * castle rights, and player to move are also stored within the pawn masks on the first and
	 * eighth ranks since pawns cannot be positioned there.
	 */
