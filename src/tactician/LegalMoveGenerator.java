...
/**
 * <p>This class generates legal and pseudo-legal moves for a given board position. A legal move is
 * simply a move under the standard rules of chess, generated by
 * {@link #legalMoves(Board)}. A pseudo-legal move is broader in the sense that it can put the
 * moving player in check. This is useful for some purposes because it's computationally expensive
 * to determine whether a player is in check. Pseudo-legal moves are generated by
 * {@link #legalMovesFast(Board, boolean)}.
 * 
...
public class LegalMoveGenerator {
	 ...
	 * {@link #attackSquaresPawnMoveWhite} and {@link #attackSquaresPawnMoveBlack} describe where
	 * a pawn can move from the indexed square. Similarly the length-64 arrays
	...
	/**
	 * Generates the attack squares for short range pieces (kings and knights, pawns are handled
	 * in {@link #initAttackSquaresPawn(Color)}.
	 ...
	 * @param attackSquares an uninitialized length-64 array modified to contain the attack squares
	 *        for a given piece
	 * @param stepSizes an array of offsets the piece can move one step, e.g. a bishop that moves
	 *        one square up and one to the left has a step size of 8 - 1 = 7
	 */
	 ...
	 * square, and step size. Consider for this example the castle ray containing f2, g3, and h4.
	 * If the opponent has a bishop or queen on any of these squares, and there is no other piece
	 * on these squares closer to e1, then white cannot castle kingside.
	 ...
	 * square, end square, and step size. Consider for this example the castle ray containing
	 * d2-d8. If the opponent has a rook or queen on any of these squares, and there is no other
	 * piece on these squares closer to d1, then white cannot castle queenside.
	 * @param color the color of the side that may want to castle
	 * @param castle the direction the player may want to castle
	 ...
	 * example if a bishop is on position 0 (the a1 square) it cannot go down and to the left with
	 * a step size of -9. If a knight is on position 8 (the a2 square) it cannot go up one square
	 * and two squares to the left with a step size of 8 - 2 = 6. That would move the knight to the
	 * g2 square, which is not a legal move for a knight.
	 * @param position the standard board position as an integer with 0 representing the a1 square,
	 *        1 representing the b1 square, 8 representing the a2 square, and so on.
	...
	/**
	 * Appends the pseudo-legal pawn moves for a given board. Optionally includes only the moves
	 * that make a capture, including en passant.
	 * @param board the board containing the position
	 * @param moves an ArrayList of pseudo-legal moves to which we will append
	 * @param capturesOnly if true will include only captures, otherwise will contain all
	 *        pseudo-legal moves
	 */
	private void appendMovesForPawn(Board board, ArrayList<Move> moves,
	...
	/**
	 * Appends the pseudo-legal moves for a long range piece for a given board. The long range
	 * pieces are the bishop, rook, and queen.
	 * @param board the board containing the position
	 * @param piece the type of piece to generate moves for
	 * @param attackSquaresTable the table containing the attack squares for the given piece, or
	 *        where the piece can move to from a given board position
	 * @param moves an ArrayList of pseudo-legal moves to which we will append
	 * @param capturesOnly if true will include only captures, otherwise will contain all
	 *        pseudo-legal moves
	 */
	 ...
	 * and that the king does not castle into a check. Note that we don't verify that the king
	 * does not castle into a check since we are only interested in pseudo-legal moves.
	 * @param board the board containing the position
	 ...
	 * nor through a check. We do not look at whether the king castles into a check since this is
	 * part of pseudo-legal move generation.
	 * @param board the board containing the position
	 ...
	 * This is useful because testing for check is computationally expensive, and pseudo-legal
	 * moves are useful for many purposes.
