...
/**
 * <p>This class is a transposition table that memoizes positions already visited. This way if we
 ...
 * The second 64-bit long uses its high 32 bits to store the fitness or node score in centipawns
 * as a float. The next highest 8 bits store the best move's source square, as it ranges from 0-63.
...
public class TranspositionTable {
	/**
	 * This enum lists the three node types that can be classified through an alpha-beta search.
	 * See {@link Brain#alphabeta(Board, int, float, float)} for more details about our use of
	 * alpha-beta pruning. PV, or principal variation, nodes are the ones where all moves had a
	 * score between alpha and beta. They are also used to determine the sequence of best moves for
	 * each player. Cut nodes are the ones where a child move scored higher than beta, or the child
	 * move is so good that the opponent would not allow the parent node to be played. All nodes
	 * are the ones where all child moves were searched but none exceeded alpha, or in other words
	 * none of the moves were good enough to affect play at the root node.
	 * 
	...
	/**
	 * This class contains entries in the transposition table. It stores the depth of the
	 * alpha-beta search, the position hash, the fitness or score of the move from the player's
	 * perspective at that node, the best move for the player at that position, and the type of
	 * node.
	 * 
	 * @author Phil Leszczynski
	 */
	public class TranspositionEntry {
		 ...
		 * @param depth the depth of the alpha-beta search remaining, or the number of steps away
		 *        from a leaf node
		...
		/**
		 * The depth of the alpha-beta search remaining, or the number of moves away from a leaf
		 * node.
		 */
		public int depth;
	...
	/**
	 * Initializes a transposition table. Note that each transposition entry is packed into a pair
	 * of 64-bit long values. See the class definition for how the packing is done.
	 * @param size the number of entries in the transposition table (Note: NOT bytes, though
	 *        bytes are often quoted when referring to a chess engine's transposition table.)
	 */
	...
	/**
	 * The array containing the hash table. Note its length is twice {@link #size} since each
	 * entry contains a pair of 64-bit long values. See the class definition for more details about
	 * how transposition entries are packed here.
	 */
	private long[] data;
}
