...
/**
 * <p>This class checks whether castling is allowed for a given side and direction. This is
 * determined through the {@link #opponentPiecePrecludesCastling(long, long)} method.
 * 
...
public class CastleRay {
	/**
	 * Initializes a castle ray with a given starting mask, ending mask, and step size. The step
	 * size is the number of steps needed to move to the right, wrapping around to the next rank,
	 * to get to each successive mask. For example the step size moving up is 8, the step size for
	 * moving to the left is -1, and the step size for moving northeast is 9.
	 ...
	 * white then we can generate the mirror image castle ray for black. For example suppose we
	 * have a castle ray for white containing the squares f1, g2, h3. Then we would return a castle
	 * ray for black containing the squares f8, g7, h6.
	...
	/**
	 * Determines whether this castle ray prevents the player from castling, given the position of
	 * an opposing piece and the positions of the other pieces on the board. If the opposing piece
	 * is not in the castle ray, then the ray cannot prevent castling. Otherwise if the other
	 * pieces on the board are not in the castle ray, then we know the ray does prevent castling.
	 * Otherwise we need to check whether the opposing piece or one of the other pieces is closer
	 * to the mask; if it's the opposing piece then castling is prevented, but if it's one of the
	 * other pieces then the ray does not prevent castling.
	 * @param oppPieceMask a 64-bit long specifying the square of the opposing piece
	 * @param otherPieces a 64-bit long representing the squares of the other pieces on the board,
	 *        see {@link Bitboard} for a summary of the long mask implementation.
